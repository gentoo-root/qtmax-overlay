...
src_prepare() {
	...
	epatch "${FILESDIR}/crossover-14.1.1-cxmenu-install-fix.patch"
}
...
src_install() {
	...
	XDG_CONFIG_DIRS="/etc/xdg" \
		...
		"${ED}opt/cxoffice/bin/cxmenu" --crossover --install --destdir "${ED}" --scope managed --menu-scope managed --verbose \
		|| die "Could not install menus"

	# TODO: make this actually work (what to do with /etc/mailcap, /etc/mime.types and weird dirs in /usr/share?)
#	# Install exe associations
#	XDG_CONFIG_DIRS="/etc/xdg" \
#		XDG_DATA_DIRS="/usr/share" \
#		"${ED}opt/cxoffice/bin/cxassoc" --crossover --install --destdir="${ED}" --scope managed --assoc-scope managed --verbose \
#		|| die "Could not install associations"
}
