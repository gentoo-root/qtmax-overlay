...
IUSE="bindist gstreamer +jit kde +minimal pgo pulseaudio selinux system-cairo system-icu system-jpeg system-sqlite test oldbranding"
...
src_unpack() {
	...
	if use oldbranding; then
		tar xof "${FILESDIR}"/firefox-21.0-branding.tar
	fi
}
...
src_prepare() {
	...
	if use kde; then
		epatch "${FILESDIR}"/01-toolkit-download-folder.${KDE_PATCH_REV}.patch
		epatch "${FILESDIR}"/02-mozilla-nongnome-proxies.${KDE_PATCH_REV}.patch
		epatch "${FILESDIR}"/03-mozilla-prefer_plugin_pref.${KDE_PATCH_REV}.patch
		epatch "${FILESDIR}"/04-mozilla-kde.${KDE_PATCH_REV}.patch
		epatch "${FILESDIR}"/05-mozilla-preferences.${KDE_PATCH_REV}.patch
		epatch "${FILESDIR}"/06-mozilla-language.${KDE_PATCH_REV}.patch
		epatch "${FILESDIR}"/07-firefox-browser-css.${KDE_PATCH_REV}.patch
		epatch "${FILESDIR}"/08-firefox-kde.${KDE_PATCH_REV}.patch
		epatch "${FILESDIR}"/09-firefox-kde-114.${KDE_PATCH_REV}.patch
		epatch "${FILESDIR}"/10-firefox-no-default-ualocale.${KDE_PATCH_REV}.patch
		epatch "${FILESDIR}"/11-firefox-branded-icons.${KDE_PATCH_REV}.patch
	fi

	if use oldbranding; then
		epatch "${FILESDIR}"/firefox-branding-about-logo-2x-remove.patch
	fi
