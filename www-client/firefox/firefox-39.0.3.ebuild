...
IUSE="bindist egl hardened kde4 kde5 +minimal neon oldbranding pgo selinux +gmp-autoupdate test"
...
RDEPEND="
	...
	kde4? ( kde-misc/kmozillahelper )
	kde5? ( kde-misc/kmozillahelper-frameworks )
...
src_prepare() {
	...
	if use kde4 || use kde5; then
		EPATCH_SUFFIX="patch" \
		EPATCH_FORCE="yes" \
		epatch "${FILESDIR}/kde-patches-${KDE_PATCH_REV}"
	fi
